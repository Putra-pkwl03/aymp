...
def edit_proposal(proposal_id):
    ...
    try:
        ...
        platform_penjualan = request.form.get('edit_platform_penjualan', '').strip()
        tahun = request.form.get('edit_tahun', '').strip()
        ...
        ''', (
            ...
            platform_penjualan, tahun, dosen_pembimbing, nid_dosen, program_studi_dosen,
...
def pengajuan_anggaran_awal_mahasiswa():
    ...
    try:
        ...
        if mahasiswa_data['status'] == 'proses':
            ...
            flash('Akun Anda sudah selesai. Anda tidak dapat mengakses halaman anggaran!', 'danger')
            cursor.close()
            return redirect(url_for('mahasiswa.proposal'))
...
def laporan_kemajuan():
    ...
    try:
        ...
        # Cek status mahasiswa
        if mahasiswa_info['status'] == 'selesai':
            flash('Akun Anda sudah selesai. Anda tidak dapat mengakses laporan kemajuan!', 'danger')
            cursor.close()
            return redirect(url_for('mahasiswa.proposal'))
        ...
        # Ambil data laporan kemajuan (tampilkan hanya setelah jadwal kemajuan mulai)
        if kemajuan_mulai_dt and now >= kemajuan_mulai_dt:
            cursor.execute(f'''
                SELECT id, kegiatan_utama, kegiatan, penanggung_jawab, target_capaian, 
                       nama_barang, kuantitas, satuan, harga_satuan, jumlah, keterangan, status, nilai_bantuan
                FROM {tabel_laporan} 
...
def laporan_akhir():
    ...
    try:
        ...
        # Cek status mahasiswa
        if mahasiswa_info['status'] == 'selesai':
            flash('Akun Anda sudah selesai. Anda tidak dapat mengakses laporan akhir!', 'danger')
            cursor.close()
            return redirect(url_for('mahasiswa.proposal'))
    ...
    try:
        ...
        cursor.execute('''
            SELECT p.nama, p.nip, p.program_studi, p.kuota_mahasiswa,
                   COUNT(DISTINCT pr.nim) as jumlah_mahasiswa_bimbing
            FROM pembimbing p
...
def delete_alat_produksi():
    ...
    try:
        ...
        return jsonify({'success': False, 'message': f'Error: {str(e)}'})
# Route untuk biaya non operasional
...
def biaya_non_operasional_mahasiswa():
    ...
    try:
        ...
        for proposal in proposals:
            ...
            # Ambil data biaya non operasional untuk proposal ini
            cursor.execute('''
...
def download_laba_rugi():
    ...
    try:
        ...
        # Ambil data proposal
        cursor.execute('''
            SELECT judul_usaha, kategori, tahun_nib
            FROM proposal
...
def laporan_arus_kas_mahasiswa():
    ...
    try:
        ...
        for proposal in proposals:
            ...
            # 4. Total Biaya Non Operasional - PER BULAN YANG DIPILIH
            cursor.execute("""
...
def tambah_proposal():
    ...
    try:
        ...
        if mahasiswa_data['status'] in ['proses', 'tolak', 'selesai']:
            ...
            flash(pesan[mahasiswa_data['status']], 'danger')
            cursor.close()
            return redirect(url_for('mahasiswa.proposal'))
        ...
        nib = request.form['nib'].strip()
        tahun_nib = request.form['tahun_nib'].strip()
        platform_penjualan = request.form['platform_penjualan'].strip().title()
        ...
        if dosen_pembimbing:
            ...
            if dosen_info and (dosen_info['kuota_mahasiswa'] - dosen_info['jumlah_mahasiswa_bimbing'] <= 0):
        ...
        # Path untuk disimpan di DB relatif ke static
        db_file_path = os.path.join('static', 'uploads', 'Proposal', safe_judul, filename)
        ...
        cursor.execute('''
            INSERT INTO proposal (
                ...
                nid_dosen, program_studi_dosen, file_path, status, tahun
            ) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        ''', (
            nim, judul_usaha, kategori, tahapan_usaha, merk_produk,
            nib, tahun_nib, platform_penjualan, dosen_pembimbing,
            nid_dosen, program_studi_dosen, db_file_path, 'draf', 
...
def delete_anggaran():
    ...
    try:
        ...
        print(f"✅ Berhasil menghapus {laporan_kemajuan_deleted} data laporan kemajuan untuk proposal {proposal_id}")
        ...
        print(f"✅ Berhasil menghapus {laporan_akhir_deleted} data laporan akhir untuk proposal {proposal_id}")
        
        # Hapus file laporan kemajuan
        app_funcs['hapus_file_laporan_kemajuan'](proposal_id, session['nim'])
        
        # Hapus file laporan akhir
        app_funcs['hapus_file_laporan_akhir'](proposal_id, session['nim'])
        ...
        message = f'Anggaran berhasil dihapus!'
...
def update_laporan_akhir():
    ...
    try:
        # Guard jadwal server-side: blokir update sebelum mulai atau setelah selesai,
        # dan auto-reject jika sudah lewat selesai (konsisten dengan halaman utama)
        try:
            auto_reject_expired_akhir_for_current_student()
            ...
            from datetime import datetime
            now_dt = datetime.now()
            if not row_guard or (not row_guard.get('akhir_mulai') and not row_guard.get('akhir_selesai')):
                return jsonify({'success': False, 'message': 'Pengaturan jadwal laporan akhir belum tersedia. Hubungi admin.'})
            if row_guard.get('akhir_mulai') and now_dt < row_guard['akhir_mulai']:
            ...
            if row_guard.get('akhir_selesai') and now_dt > row_guard['akhir_selesai']:
                return jsonify({'success': False, 'message': 'Jadwal laporan akhir telah berakhir. Perubahan tidak dapat dilakukan.'})
        except Exception:
            # Jika guard gagal, tetap lanjutkan validasi biasa agar tidak mematikan alur; namun idealnya ada jadwal.
            pass
        ...
        update_params = (kuantitas, harga_satuan, jumlah, db_path, status, laporan_id)
        ...
        if rows_affected > 0:
            ...
            if (final_verification and 
                ...
                final_verification['status'] == status):
                print("=== DEBUG: Data berhasil tersimpan dengan benar ===")
                return jsonify({'success': True, 'message': 'Data laporan akhir berhasil diperbarui dan status diubah menjadi diajukan!'})
            else:
...
def mahasiswa_download_laporan_kemajuan(proposal_id):
    ...
    try:
        ...
        # Ambil path file laporan kemajuan
        cursor.execute('''
            SELECT laporan_kemajuan_path 
            FROM proposal 
            WHERE id = %s AND nim = %s
        ''', (proposal_id, session['nim']))
    ...
    try:
        ...
        if mahasiswa_info:
            ...
            app_funcs['log_mahasiswa_activity'](
                ...
                f'id_{biaya_id}', 
                f'Menambahkan biaya non operasional: {nama_biaya}',
                None,
                data_baru
            )
        ...
        return jsonify({'success': True, 'message': 'Biaya non operasional berhasil ditambahkan'})
...
def get_biaya_non_operasional(biaya_id):
    ...
    try:
        ...
        biaya = cursor.fetchone()
        if not biaya:
            return jsonify({'success': False, 'message': 'Biaya non operasional tidak ditemukan atau bukan milik Anda'})
    ...
    try:
        ...
        if mahasiswa_info:
            ...
            app_funcs['log_mahasiswa_activity'](
                ...
                'biaya_non_operasional', 
                f'id_{biaya_id}', 
                f'Mengupdate biaya non operasional: {nama_biaya}',
                biaya_exists,
                data_baru
            )
        ...
        return jsonify({'success': True, 'message': 'Biaya non operasional berhasil diupdate'})
    ...
    try:
        ...
        if mahasiswa_info:
            app_funcs['log_mahasiswa_activity'](
                ...
                'biaya_non_operasional', 
                f'id_{biaya_id}', 
                f'Menghapus biaya non operasional: {biaya_exists["nama_biaya"]}',
                biaya_exists,
                None
            )
        ...
        return jsonify({'success': True, 'message': 'Biaya non operasional berhasil dihapus'})
        ...
        if format_type == 'excel':
            ...
            if file_buffer:
                ...
                if mahasiswa_info_session:
                    app_funcs['log_mahasiswa_activity'](
                        mahasiswa_info_session['id'],
                        mahasiswa_info_session['nim'],
                        mahasiswa_info_session['nama_ketua'],
                        'download',
                        'laporan_neraca',
                        f'proposal_{proposal_id}',
                        f'Download laporan neraca format {format_type.upper()}',
                        {'format': format_type, 'proposal_id': proposal_id},
                        None
                    )
...
def upload_file_laporan_kemajuan():
    ...
    try:
        # Guard jadwal: hanya boleh upload saat jadwal kemajuan sudah mulai
        try:
            cursor_guard = app_funcs["mysql"].connection.cursor(MySQLdb.cursors.DictCursor)
            cursor_guard.execute('SELECT kemajuan_mulai FROM pengaturan_jadwal ORDER BY id DESC LIMIT 1')
            row_guard = cursor_guard.fetchone()
            cursor_guard.close()
            if not row_guard or not row_guard.get('kemajuan_mulai') or datetime.now() < row_guard['kemajuan_mulai']:
                return jsonify({'success': False, 'message': 'Upload dibuka saat jadwal kemajuan mulai. Silakan tunggu jadwal dimulai.'})
        except Exception:
            return jsonify({'success': False, 'message': 'Pengaturan jadwal tidak tersedia. Hubungi admin.'})
...
def edit_file_laporan_kemajuan():
    ...
    try:
        # Guard jadwal: hanya boleh edit (reupload) saat jadwal kemajuan sudah mulai
        try:
            cursor_guard = app_funcs["mysql"].connection.cursor(MySQLdb.cursors.DictCursor)
            cursor_guard.execute('SELECT kemajuan_mulai FROM pengaturan_jadwal ORDER BY id DESC LIMIT 1')
            row_guard = cursor_guard.fetchone()
            cursor_guard.close()
            if not row_guard or not row_guard.get('kemajuan_mulai') or datetime.now() < row_guard['kemajuan_mulai']:
                return jsonify({'success': False, 'message': 'Edit/upload dibuka saat jadwal kemajuan mulai. Silakan tunggu jadwal dimulai.'})
        except Exception:
            return jsonify({'success': False, 'message': 'Pengaturan jadwal tidak tersedia. Hubungi admin.'})
...
def upload_file_laporan_akhir():
    ...
    try:
        # Guard jadwal: hanya boleh upload saat jadwal laporan akhir sudah mulai
        try:
            cursor_guard = app_funcs["mysql"].connection.cursor(MySQLdb.cursors.DictCursor)
            cursor_guard.execute('SELECT akhir_mulai FROM pengaturan_jadwal ORDER BY id DESC LIMIT 1')
            row_guard = cursor_guard.fetchone()
            cursor_guard.close()
            if not row_guard or not row_guard.get('akhir_mulai') or datetime.now() < row_guard['akhir_mulai']:
                return jsonify({'success': False, 'message': 'Upload laporan akhir dibuka saat jadwal laporan akhir mulai. Silakan tunggu jadwal dimulai.'})
        except Exception:
            return jsonify({'success': False, 'message': 'Pengaturan jadwal tidak tersedia. Hubungi admin.'})
...
def get_detail_penilaian_mahasiswa():
    ...
    try:
        ...
        cursor.execute('''
            SELECT m.nama_mahasiswa, m.nim_mahasiswa, p.judul_usaha
            FROM mahasiswa m
            LEFT JOIN proposal p ON m.nim = p.nim
            WHERE m.nim = %s
        ''', (nim,))
        ...
        if penilaian:
            cursor.execute('''
                SELECT dp.*, p.pertanyaan, p.skor_maksimal
                FROM detail_penilaian_mahasiswa dp
                LEFT JOIN pertanyaan_penilaian_mahasiswa p ON dp.id_pertanyaan = p.id
                WHERE dp.id_penilaian_mahasiswa = %s
                ORDER BY p.id
            ''', (penilaian['id'],))
...
def get_detail_penilaian_proposal():
    ...
    try:
        ...
        # Ambil data mahasiswa
        cursor.execute('''
            SELECT m.nama_mahasiswa, m.nim, p.judul_usaha
            FROM mahasiswa m
            LEFT JOIN proposal p ON m.nim = p.nim
            WHERE m.nim = %s
        ''', (nim,))
        ...
        cursor.execute('''
            SELECT pp.*, r.nama_reviewer
            FROM penilaian_proposal pp
            LEFT JOIN reviewer r ON pp.id_reviewer = r.id
        ...
        # Ambil catatan dan rekomendasi
        catatan = []
        rekomendasi = []
        if penilaian:
            cursor.execute('''
                SELECT catatan FROM catatan_penilaian_proposal 
                WHERE id_penilaian_proposal = %s
            ''', (penilaian['id'],))
            catatan = [row['catatan'] for row in cursor.fetchall()]
            
            cursor.execute('''
                SELECT rekomendasi_nilai_bantuan FROM rekomendasi_penilaian_proposal 
                WHERE id_penilaian_proposal = %s
            ''', (penilaian['id'],))
            rekomendasi = [{'rekomendasi': row['rekomendasi_nilai_bantuan']} for row in cursor.fetchall()]
...
def get_detail_penilaian_kemajuan():
    ...
    try:
        ...
        cursor.execute('''
            SELECT pk.*, r.nama_reviewer
            FROM penilaian_laporan_kemajuan pk
            LEFT JOIN reviewer r ON pk.id_reviewer = r.id
        ...
        if penilaian:
            cursor.execute('''
                SELECT catatan FROM catatan_penilaian_laporan_kemajuan 
                WHERE id_penilaian_laporan_kemajuan = %s
            ''', (penilaian['id'],))
            catatan = [row['catatan'] for row in cursor.fetchall()]
            
            cursor.execute('''
                SELECT rekomendasi_pendanaan, alasan_rekomendasi 
                FROM rekomendasi_penilaian_laporan_kemajuan 
                WHERE id_penilaian_laporan_kemajuan = %s
            ''', (penilaian['id'],))
...
def get_detail_penilaian_akhir():
    ...
    try:
        ...
        cursor.execute('''
            SELECT pa.*, r.nama_reviewer
            FROM penilaian_laporan_akhir pa
            LEFT JOIN reviewer r ON pa.id_reviewer = r.id
        ...
        if penilaian:
            cursor.execute('''
                SELECT catatan FROM catatan_penilaian_laporan_akhir 
                WHERE id_penilaian_laporan_akhir = %s
            ''', (penilaian['id'],))
            catatan = [row['catatan'] for row in cursor.fetchall()]
            
            cursor.execute('''
                SELECT rekomendasi_kelulusan, alasan_rekomendasi 
                FROM rekomendasi_penilaian_laporan_akhir 
                WHERE id_penilaian_laporan_akhir = %s
            ''', (penilaian['id'],))
            rekomendasi = cursor.fetchall()
...
def bimbingan():
    ...
    if not hasattr(app_funcs['mysql'], 'connection') or app_funcs['mysql'].connection is None:
        flash('Koneksi ke database gagal. Cek konfigurasi database!', 'danger')
        return render_template('mahasiswa/bimbingan.html', riwayat_bimbingan=[])
