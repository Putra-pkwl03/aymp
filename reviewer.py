...
def proposal():
    ...
    try:
        ...
        return render_template('reviewer/proposal.html', proposals=proposals)
...
def update_nilai_bantuan():
    ...
    try:
        ...
        # Update status proposal jika ada perubahan status anggaran
        if status_anggaran_baru == 'revisi':
            cursor.execute('''
                UPDATE proposal_reviewer 
                SET status_review = 'sedang_direview'
                WHERE id_proposal = %s AND id_reviewer = %s
            ''', (proposal_id, reviewer_id))
...
def send_proposal_to_admin():
    ...
    try:
        ...
        # Update status review proposal menjadi selesai
        cursor.execute('''
            UPDATE proposal_reviewer 
            SET status_review = 'selesai_review', tanggal_review = NOW()
            WHERE id_proposal = %s AND id_reviewer = %s
        ''', (proposal_id, reviewer_id))
...
def penilaian(proposal_id):
    ...
    try:
            ...
            if jadwal and jadwal.get('reviewer_proposal_selesai'):
                ...
                if datetime.now() > jadwal['reviewer_proposal_selesai']:
                    ...
                    if not exists:
                        ...
                        cursor.execute('''
                            SELECT id, bobot, skor_maksimal FROM pertanyaan_penilaian_proposal WHERE is_active = TRUE ORDER BY created_at ASC
                        ''')
                        pertanyaan_zero = cursor.fetchall()
                        ...
                        for p in pertanyaan_zero:
                            ...
                            ''', (proposal_id, reviewer_id, p['id'], 0, p['bobot'], 0))
        ...
        # Ambil data pertanyaan penilaian dari database
        cursor.execute('''
        ...
        pertanyaan_penilaian = cursor.fetchall()
...
def save_penilaian():
    ...
    try:
        ...
        nilai_akhir = (penilaian['totalNilai'] / total_bobot * 100) if total_bobot > 0 else 0
...
def daftar_mahasiswa_laporan_kemajuan():
    ...
    try:
        ...
        return render_template('reviewer/daftar_mahasiswa_laporan_kemajuan.html', proposals=proposals)
...
def laporan_kemajuan_awal_bertumbuh(proposal_id):
    ...
    try:
        ...
        # Hitung total nilai bantuan dari data yang disetujui
        cursor.execute(f'''
            SELECT COALESCE(SUM(jumlah), 0) as total_nilai_bantuan
            FROM {tabel_laporan_kemajuan}
            WHERE id_proposal = %s AND status = 'disetujui'
        ''', (proposal_id,))
        ...
        cursor.execute(f'''
            SELECT COALESCE(SUM(jumlah), 0) as total_laporan_kemajuan_disetujui
            FROM {tabel_laporan_kemajuan}
            WHERE id_proposal = %s AND status = 'disetujui'
        ''', (proposal_id,))
...
def get_pertanyaan_penilaian_laporan_kemajuan():
    ...
    try:
        ...
        cursor.execute('''
            SELECT * FROM pertanyaan_penilaian_laporan_kemajuan 
            WHERE status = 'Aktif' 
            ORDER BY id
        ''')
        
        pertanyaan = cursor.fetchall()
        cursor.close()
...
def simpan_penilaian_laporan_kemajuan():
    ...
    try:
        ...
        # Hitung nilai akhir seperti IPK
        # Konversi ke float untuk memastikan perhitungan yang benar
        ...
        nilai_akhir = (total_nilai / total_bobot * 100) if total_bobot > 0 else 0
        ...
        for i, detail in enumerate(detail_penilaian):
            ...
            nilai_terbobot = float(detail['nilai_terbobot'])
...
def daftar_mahasiswa_laporan_akhir():
    ...
    try:
        ...
        return render_template('reviewer/daftar_mahasiswa_laporan_akhir.html', proposals=proposals)
...
def laporan_akhir_awal_bertumbuh(proposal_id):
    ...
    try:
        ...
        # Hitung total laporan akhir
        total_laporan_akhir = sum(row['jumlah'] for row in anggaran_data) if anggaran_data else 0
        
        # Hitung total nilai bantuan dari laporan kemajuan yang disetujui
        cursor.execute('''
            SELECT 
                (COALESCE((SELECT SUM(jumlah) FROM laporan_kemajuan_awal WHERE id_proposal = %s AND status = 'disetujui'), 0) +
                 COALESCE((SELECT SUM(jumlah) FROM laporan_kemajuan_bertumbuh WHERE id_proposal = %s AND status = 'disetujui'), 0)) as total_nilai_bantuan
        ''', (proposal_id, proposal_id))
            ...
            if jadwal and jadwal.get('reviewer_akhir_selesai'):
                ...
                if datetime.now() > jadwal['reviewer_akhir_selesai']:
                    ...
                    if not ex:
                        ...
                        # detail 0
                        cursor2.execute('SELECT id, bobot, skor_maksimal FROM pertanyaan_penilaian_laporan_akhir WHERE status="Aktif" ORDER BY urutan ASC')
                        pts = cursor2.fetchall()
                        for p in pts:
                            cursor2.execute('''
                                ...
                                VALUES (%s,%s,%s,%s,%s,%s)
                            ''', (pen_id, p['id'], 0, p['bobot'], p['skor_maksimal'], 0))
                        get_app_functions()['mysql'].connection.commit()
            cursor2.close()
        except Exception:
            try:
...
def get_pertanyaan_penilaian_laporan_akhir():
    ...
    try:
        ...
        cursor.execute('''
            SELECT * FROM pertanyaan_penilaian_laporan_akhir 
            WHERE status = 'Aktif' 
            ORDER BY urutan ASC
        ''')
    ...
    try:
        ...
        for detail in detail_penilaian:
            ...
            ''', (penilaian_id, detail['id_pertanyaan'], detail['skor_diberikan'], 
                  detail['bobot_pertanyaan'], detail['skor_maksimal'], detail['nilai_terbobot']))
