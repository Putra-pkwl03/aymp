...
def daftar():
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            return render_template('logdaf/daftar.html', program_studi_list=program_studi_list)
        cursor.execute('SELECT * FROM mahasiswa WHERE nim = %s OR email = %s', (nim, email))
...
def update_arus_kas_otomatis(proposal_id, bulan_tahun):
    ...
    Dipanggil setiap kali ada perubahan di: penjualan, produksi, biaya operasional, biaya non operasional, alat produksi
    """
    try:
        if not hasattr(mysql, 'connection') or mysql.connection is None:
            return False
